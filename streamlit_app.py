...
with col1:
    ...
    if a_type == "공사":
        ...
        report_text = (
            f"[{tunnel_name}]\n\n"
            f"{disp_direction} {work_name}{lane_str} 차단\n"
            "안전운전하세요."
        )
...
with col1:
    ...
    if a_type == "공사":
        ...
        # ✅ SyntaxError 방지: f-string 중첩 제거(미리 변수로 만들기)
        pos_lane = f" {lane}" if lane else ""
        pos_dist = f" {dist}m" if dist else ""
...
ㅇ일시 : {time_str}분경
ㅇ최초인지 : {detect_way}
ㅇ위치 : {tunnel_name} {loc_detail}{pos_lane}{pos_dist} ({disp_direction})
ㅇ관리 : {manager}
